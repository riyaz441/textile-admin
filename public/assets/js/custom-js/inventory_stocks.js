$(document).ready(function () {
    // datatable js start
    new DataTable('#inventory-stocks-table', {
        layout: {},
        "ordering": false,
        oLanguage: {
            sLengthMenu: "_MENU_",
    ...
    // active and in active status changes start
    $(document).on('change', '.change_inventory_stock_status', function () {
        let dataId = $(this).data('id');
        let isChecked = $(this).is(':checked');
        let status = isChecked ? 'Active' : 'Inactive';
        $.ajax({
            url: APP_URL + '/change_inventory_stock_status',
            type: 'GET',
    ...
    // active and in active status changes end
});
